...
class MaleIdol extends Idol {
    ...
    sing() {
        return `${this.name}(${this.year})이 노래를 합니다.`
    }
}
...
let female = iveMembers.map(member => new FemaleIdol(member.name, member.year));
let male = btsMembers.map(member => new MaleIdol(member['name'], member['year']));
//console.log(female);
//console.log(male);

let femaleGroup = new IdolGroup('아이브', female);
let maleGroup = new IdolGroup('BTS', male);
//console.log(maleGroup);
//console.log(femaleGroup);

let country = new Country('대한민국', [femaleGroup, maleGroup]);
//console.log(country);

for(group of country.idolGroups) {
    console.log(`[${group.name} - ${country.name}]`);
